...
double get_autocorr(std::vector<std::complex<double>> &timeseries, std::vector<std::complex<double>> &autocorrelation)  // Das hier wird typischerweise die g2-funktion darstellen
	{
...
double get_autocorr_real(std::vector<double> &realtimeseries, std::vector<double> &realautocorrelation)  // Das hier wird typischerweise die g2-funktion darstellen
	{
	 fftw_complex *in_ts, *out_spec, *power_spec, *autocorr; 
...
double get_autocorr(std::vector<double> &realtimeseries, std::vector<double> &autocorrelation)
	{	
...
std::vector<double> get_crosscorr(std::vector<std::complex<double>> &timeseries1, std::vector<std::complex<double>> &timeseries2, std::vector<std::complex<double>> &crosscorrelation)  // Das hier wird typischerweise die g2-funktion darstellen
	{
...
std::vector<double> get_crosscorr_real(std::vector<double> &realtimeseries1, std::vector<double> &realtimeseries2, std::vector<double> &crosscorrelation)  // Das hier wird typischerweise die g2-funktion darstellen
	{
	 fftw_complex *in1_ts, *in2_ts, *out1_spec, *out2_spec, *corr_spec, *crosscorr; 
...
std::vector<double> get_crosscorr(std::vector<double> &Realtimeseries1, std::vector<double> &Realtimeseries2, std::vector<double> &g2)
	{
...
void get_g2(std::vector<double> &Timeseries, std::vector<double> &g2)
	{
...
void get_g2(std::vector<double> &Timeseries1, std::vector<double> &Timeseries2, std::vector<double> &g2)
	{
...
void print_correlation(std::vector<double> &correlation, Time dt, std::string correlationoutname, std::size_t maxN , std::size_t every, std::string unit)
...
void print_correlation(std::vector<std::complex<double>> &correlation, Time dt, std::string correlationoutname, std::size_t maxN, std::size_t every, std::string unit)
	{	std::ofstream corr_out (correlationoutname, std::ios::out | std::ios::trunc);      //Output File (ofstream) overwrite
		std::cout << "Write file: "<< correlationoutname << std::endl;
		size_t correlation_size= correlation.size();
		if(maxN>correlation_size || maxN==0) maxN=correlation_size;
...
void print_correlations(std::vector<std::vector<double>> &correlation, Time dt, std::string correlationoutname, std::string correlationheader, std::size_t maxN, int every, unsigned int precision)
	{	std::ofstream corrout (correlationoutname, std::ios::out | std::ios::trunc);      //Output File (ofstream) overwrite
		...
		for(int iter= 0; iter<correlation.size(); iter+=every )  
			{corrout << iter*dt;
				for(int elem=0; elem<correlation[0].size(); elem++)
					{corrout << "\t" << correlation[iter][elem];}
				corrout << std::endl;
			}					
		corrout.close();
