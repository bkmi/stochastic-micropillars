...
#define MODELSRC "muPillar_Pulse.cc"
#define MODELPATH "muPillar_Pulse/"
#define PI 3.1415
...
//----- BEGIN PARAMETERS
	...
	Time t_turnon=	katana::getCmdOption(argv, argv+argc, "-t_turnon", 20)*1e-9;
	Time t_trysteady=katana::getCmdOption(argv, argv+argc, "-t_trysteady", 15)*1e-9;
	Time dt = 	katana::getCmdOption(argv, argv+argc, "-dt", 0.0005)*1e-9;// calc in s, measure ns
	
	double J=0;	
	double jstart=katana::getCmdOption(argv, argv+argc, "-jstart" , 1.0)*1e-6; 			
	double jstop=katana::getCmdOption(argv, argv+argc, "-jstop" , 10)*1e-6; 			
	double jincr=katana::getCmdOption(argv, argv+argc, "-jincr" , 0.3)*1e-6; //Input in numbers of the Jth

	size_t corrprlen=int(katana::getCmdOption(argv, argv+argc, "-corrprintlen", 20)/dt);	
	size_t fourierlen=int(katana::getCmdOption(argv, argv+argc, "-fourierprintlen", 20));	
...
//----- VARS & INIT CONDITIONS ----------- // 
	...
	X_init[0]=katana::getCmdOption(argv,argv+argc, "-Esinit_Re", 0.1);
	X_init[1]=katana::getCmdOption(argv,argv+argc, "-Esinit_Im", 0.1);
	X_init[2]=katana::getCmdOption(argv,argv+argc, "-Ewinit_Re", 0.1);
	X_init[3]=katana::getCmdOption(argv,argv+argc, "-Ewinit_Im", 0.1);
	X_init[4]=katana::getCmdOption(argv,argv+argc, "-Rhoinit", 0.1);
	X_init[5]=katana::getCmdOption(argv,argv+argc, "-Ninit", 0.1);
...
//----- MODEL PARAMS --------------------//
	...
	auto kappa_w    = katana::getCmdOption(argv,argv+argc, "-kappa_w", 43)*(1/1e-9);	//ps^-1 -> 1/s
	auto mu_s 	= 3.75*(1e-9*e0);	//nm * e0 -> m*C
	auto mu_w 	= 3.70*(1e-9*e0);	//nm * e0 -> m*C
	auto _epsi_ss   = katana::getCmdOption(argv, argv+argc, "-gss", 27)*1e-10;		//m^2 A^-1 V-^1
	auto _epsi_ww   = katana::getCmdOption(argv, argv+argc, "-gww", 30)*1e-10;		//m^2 A^-1 V^-1
	auto _epsi_sw   = katana::getCmdOption(argv, argv+argc, "-gsw", 40)*1e-10;		//m^2 A^-1 V^-1
	auto _epsi_ws   = katana::getCmdOption(argv, argv+argc, "-gws", 16)*1e-10;		//m^2 A^-1 V^-1
	auto beta	= katana::getCmdOption(argv, argv+argc, "-beta", 0.014);
	auto eta 	= katana::getCmdOption(argv, argv+argc, "-eta", 505.0)*1e-3;
	auto tau_r 	= katana::getCmdOption(argv, argv+argc, "-tau_r", 1.)*(1e-9); 	//ps -> s
	auto S_in 	= katana::getCmdOption(argv, argv+argc, "-S_in", 1E-5);//m^2 ps^-1 -> m^2 s^-1
	auto dS_in 	= katana::getCmdOption(argv, argv+argc, "-dS_in", 0.0);//m^2 ps^-1 -> m^2 s^-1
	auto V	 	= 4*1E-18;// 0.5*6.3*(1E-6)*1E-6*1E-6; 	//micro m^3 -> m^3
	auto A		= 12* 1E-12; // 3.14*(1E-6)*1E-6; 	//micro m^2
	auto Z_QD 	= katana::getCmdOption(argv, argv+argc, "-ZQD", 250.);
	auto Z_inac	= katana::getCmdOption(argv, argv+argc, "-Z_inac", 750.);
	auto n_bg 	= 3.34;
	auto epsi_bg	= n_bg*n_bg;
	auto tau_sp 	= katana::getCmdOption(argv, argv+argc, "-tau_sp", 0.11)*(1e-9);		//ns -> s
	auto T_2 	= 0.33*(1e-12); 	//ps -> s
	auto hbar_omega	= 1.38*(1.6e-19); 	//eV -> J
	auto _epsi_tilda = epsi0*n_bg*c0;
	auto alpha	=2.;
	Time TiSa_center= katana::getCmdOption(argv, argv+argc, "-TiSa_center", 5)*(1e-9);
	Time TiSa_width	= katana::getCmdOption(argv, argv+argc, "-TiSa_width", 0.50)*(1e-9);
	double TiSa_height = katana::getCmdOption(argv, argv+argc, "-TiSa_height", 1.0)*(1e-6);
...
//----- BEGIN FILEHEADERS ---------------//
	...
	traceheader_stringstream << "#Time [ns] \t Re(Es) \t Im(Es) \t Re(Ew) \t Im(Ew) \t rho \t w";
	std::string traceheader(traceheader_stringstream.str());
	std::string sweepfileheader="\n#J \t |Es|^2 \t |Ew|^2 \t rho \t w  \t S_tot";
	std::string avgtraceheader="#Time [ns] \t Intensity";
...
//----- END FILL DELAYVECTOR ------------//
	...
	auto apply_pulse=[&](Time &T, Time width, double height, Time center)
		{
			return height*std::exp(-0.5*(T-center)*(T-center)/(width*width));
	...
	auto dde_equation = [&](State &X, State &X_tau, Time &T, State &dxdt) 
		{
			...
			auto TiSa=apply_pulse(T, TiSa_width, TiSa_height, TiSa_center);
			if(sponE) {dxdtEs_sp=std::sqrt(beta*rho*rho/tau_sp*PF_E/dt)*gwN1;} else dxdtEs_sp=beta*rho*rho/tau_sp*PF_E/std::conj(Es);
			if(sponE) {dxdtEw_sp=std::sqrt(beta*rho*rho/tau_sp*PF_E/dt)*gwN2;} else dxdtEw_sp=beta*rho*rho/tau_sp*PF_E/std::conj(Ew);
			...
			double S=S_in+dS_in*w;
			double rhoinac=-w*tau_sp*S+std::sqrt(w*w*tau_sp*tau_sp*S/4.0+w*tau_sp*S); //bimolekulare rekombination
			...
			dxdtrho=-(2*rho-1)*(gs*std::norm(Es)+gw*std::norm(Ew))+S*w*(1-rho)-rho*rho/tau_sp ;
			dxdtw=pump_factor*(J+TiSa)-S*2*Z_QD/A*w*(1-rho)-S*(2*Z_inac/A)*rhoinac*rhoinac/tau_sp - w/tau_r ;
...
//----- PRINT HEADER OF CONFIG/SWEEPFILE ---------------------------------- //
	...
	outputfile << "# Model from RED16, PLMCN version, with off-resonant injection of a TiSa Pulse, project started: 2017/04/11 : last updated: 2017/04/11" << std::endl;
...
//------ BEGIN PREPARATION ----------------------------//
	...
	katana::container_1d avgstate, avgacs, avgacw, avgcc, avgspecs, avgspecw, deltaOmcontainer, avgInts, avgIntw;
...
//----- Lambda function for output, easier to implement inside the main() because we already have declared State, Time and Delayvector type		
	...
	auto output_Int_Fields = [&](State &X, State &X_tau, Time &T, State &dxdt) 
		{	double Ints=X[0]*X[0]+X[1]*X[1], Intw=X[2]*X[2]+X[3]*X[3], rho=X[4], w=X[5], S_tot=S_in*w;
			...
			avgstate.add({Ints, Intw, rho, w, S_tot});
		};
	...
	for(J=jstart; J<=jstop; J=J+jincr)
		{//---- BEGIN PREPARATION ------------------------------//
			std::string trace_outname, avgtraces_outname, avgtracew_outname, acs_outname, acw_outname, cc_outname, specs_outname, specw_outname;
			char J_buf[10]; 	std::sprintf(J_buf , "%06.2f", J*1E6);
			trace_outname= path +  J_buf + "_muA_" + turnontsfilename; 
			avgtraces_outname= path +  J_buf + "_muA_s_" + tsfilename; 
			avgtracew_outname= path +  J_buf + "_muA_w_" + tsfilename; 
			...
			avgstate.clear(); avgacs.clear(); avgacw.clear(); avgcc.clear(), deltaOmcontainer.clear(); // Clear all container
		//----- END PREPAREATION _------------------------------//
		//----- MEASURE TURNON ---------------------------------//
			X=X_init;
			...
			katana::dump_trace(Full_Ttrace ,dt, trace_outname, traceheader, 5);
		//----- END TURNON ------------------------------------//
		...
		//----- BEGIN MEASURING! ----------//
			for(int iter=0; iter<makeavgs; ++iter)
				...
				acs=katana::get_g2(Ints_trace);			avgacs.add(acs); avgInts.add(Ints_trace);
				acw=katana::get_g2(Intw_trace);		avgacw.add(acw); avgIntw.add(Intw_trace);
				cc=katana::get_g2_cc(Ints_trace, Intw_trace);	avgcc.add(cc);
				};							
		//----- END MEASURING ----//

		//----- BEGIN OUTPUT -----//
			std::vector<double> avg_vec=avgstate.get_avg(), avg_cc;  avgcc.get_avg(avg_cc);
			...
			std::vector<double> avg_Ints; avgInts.get_avg(avg_Ints);		std::vector<double> avg_Intw; avgIntw.get_avg(avg_Intw);


			outputfile << J*1E6 << '\t' << avg_vec[0] << '\t' << avg_vec[1] << '\t' << avg_vec[2] << '\t' << avg_vec[3]  << '\t' << avg_vec[4] << std::endl;
			...
			arma::mat A;
			katana::add_vec_to_matrix(A, avg_Ints);
			katana::add_vec_to_matrix(A, avg_Intw);
			std::vector<std::vector<double>> avg_Intensities=katana::mat_to_stdvecvec(A);
			arma::mat B=katana::stdvecvec_to_mat(avg_Intensities);
			if(arma::approx_equal(A,B, "reldiff", 0.0001)) std::cout << "YES!" << std::endl;	
			katana::dump_trace(avg_Intensities, dt, avgtraces_outname, "#Time [ns] \t Int_s \t Int_w \n", 1);
