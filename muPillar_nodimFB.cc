...
//----- BEGIN PARAMETERS
	...
	Time t_turnon=	katana::getCmdOption(argv, argv+argc, "-t_turnon", 10);
	Time t_trysteady=katana::getCmdOption(argv, argv+argc, "-t_trysteady", 50);
...
//----- MODEL PARAMS --------------------//
	...
	//-- Laser parameters
	auto kappa_s    = katana::getCmdOption(argv,argv+argc, "-kappa_s", 0.039)*(1/1e-12);	//ps^-1 -> 1/s
	auto kappa_w    = katana::getCmdOption(argv,argv+argc, "-kappa_w", 0.042)*(1/1e-12);	//ps^-1 -> 1/s
	...
	auto dde_equation = [&](State &X, State &X_tau, Time &T, State &dxdt) 
		{
			...
			if(sponE) {dxdtEs_sp=std::sqrt(eps_ss*beta*rho*PF_E/dt)*gwN1;} else dxdtEs_sp=eps_ss*beta*rho*PF_E/std::conj(Es);
			if(sponE) {dxdtEw_sp=std::sqrt(eps_ss*beta*rho*PF_E/dt)*gwN2;} else dxdtEw_sp=eps_ss*beta*rho*PF_E/std::conj(Ew);
			auto gs = (std::norm(mu_s/hbar))*T_2*0.5/(1.+std::norm(Es)+ eps_sw/eps_ss*std::norm(Ew));
			auto gw = (std::norm(mu_w/hbar))*T_2*0.5/(1.+eps_ws/eps_ss*std::norm(Es)+eps_ww/eps_ss *std::norm(Ew));
			double rhoinac=w/(1+w); //tau_sp*tau_sp*R*R*w/(1+tau_sp*tau_sp*R*R*w);


			dxdtEs=0.5*tau_sp*PF_E*gs*(2*rho-1)*(double(1)-ii*alpha)*Es-tau_sp*kappa_s*(Es-f_ss*Es_tau)+dxdtEs_sp-ii*deltaOmega_s*Es;
			dxdtEw=0.5*tau_sp*PF_E*gw*(2*rho-1)*(double(1)-ii*alpha)*Ew-tau_sp*kappa_w*(Ew-f_ww*Ew_tau)+dxdtEw_sp-ii*deltaOmega_w*Ew;
			dxdtrho=-tau_sp/(eps_ss)*(2*rho-1)*(gs*std::norm(Es)+gw*std::norm(Ew))+w*(1-rho)-rho;
			dxdtw=pump_factor*(J)-R*w*(1-rho)-tau_sp*S_in*(2*Z_inac/A)*rhoinac - (tau_sp/tau_r)*w ;
...
//------ BEGIN PREPARATION ----------------------------//
	...
	katana::Avg_container avgstate, avgacs, avgacw, avgcc, avgspecs, avgspecw, deltaOmcontainer;
...
//---- DOING J-SWEEP  ------- //
	...
	for(J=jstart; J<=jstop; J=J+jincr)
		{//---- BEGIN PREPARATION ------------------------------//
			std::string trace_outname, acs_outname, acw_outname, cc_outname, specs_outname, specw_outname;
			char J_buf[10]; 	std::sprintf(J_buf , "%06.2f", J*1E6);
			trace_outname= path +  J_buf + "_Jth_" + turnontsfilename; 
			acs_outname= path +  J_buf + "_Jth_" + "auto_s_" +g2filename;
			acw_outname= path +  J_buf + "_Jth_" + "auto_w_" +g2filename;
			cc_outname= path +  J_buf + "_Jth_" + "cross_sw_" +g2filename;
			specs_outname= path +  J_buf + "_Jth_" + "optspec_s_" +specfilename;
			specw_outname= path +  J_buf + "_Jth_" + "optspec_w_" +specfilename;
		...
		//----- BEGIN OUTPUT -----//
			...
			katana::print_correlation(avg_acs ,dt, acs_outname, corrprlen, 5, "ns");
			katana::print_correlation(avg_acw ,dt, acw_outname, corrprlen, 5, "ns");
			katana::print_correlation(avg_cc ,dt, cc_outname, corrprlen, 5, "ns");
			katana::dump_optical_spec(avg_outspec_s, cutfouriertrace*dt, (int) fourierlen*(t_stop-t_start), specs_outname); //most of the spectrum is shit, only first X elem. dumped
			katana::dump_optical_spec(avg_outspec_w, cutfouriertrace*dt, (int) fourierlen*(t_stop-t_start), specw_outname);
//---------------------END OUTPUT ------//
		}
//------END SWEEP! ---//	
